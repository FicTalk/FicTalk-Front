...
const likePostAPI = async (url: string) => await fetch(url, { method: "POST" }).then((res) => res.json());
/**PUT */
...
const likeDeleteAPI = async (url: string) => await fetch(url, { method: "DELETE" }).then((res) => res.json());
...
export function useLikePost() {
	const { id } = useParams();
	return useSWRMutation(`/api/post/${id}`, async () => await likePostAPI(`/api/post/like/${id}`));
}
export function useUnLikePost() {
	const { id } = useParams();
	return useSWRMutation(`/api/post/${id}`, async () => await likeDeleteAPI(`/api/post/like/${id}`));
}
