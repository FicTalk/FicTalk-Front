...
const CreatePostBtn = ({ data, error, isLoading, trigger, isMutating }: Props) => {
	...
	if (error)
		return (
			<button onClick={() => toast.error("에러가 발생했습니다.", { duration: 2000 })} className="group p-1 flex flex-col gap-0.5">
				<FilePen className="group-hover:text-black text-black/70 transition mx-auto" />
				<p className="text-xs group-hover:text-black text-black/70 transition text-center">글쓰기</p>
			</button>
		);
	...
	return data ? (
		...
		<button onClick={() => toast.error("로그인이 필요한 서비스입니다.", { duration: 2000 })} className="group p-1 flex flex-col gap-0.5">
			<FilePen className="group-hover:text-black text-black/70 transition mx-auto" />
			<p className="text-xs group-hover:text-black text-black/70 transition text-center">글쓰기</p>
		</button>
	);
