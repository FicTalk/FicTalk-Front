...
function WebtoonList() {
	...
	const renderAlertButton = useCallback(
		(item: Webtoon) => (
			<button onClick={() => handleAlertToggle(item)} disabled={alertMap.has(item.id) ? deleteMutating : createMutating} aria-label="알람 설정 토글">
...
function ComboBoxes() {
	...
	return selectRole.map((list) => (
		<Combobox key={list.role.value} list={list.content} role={list.role} defaultValue={defaultValue(list.role.value)} onChange={onChange} />
	));
}
