...
function WebtoonList() {
  ...
  const { trigger: createAlert } = useCreateAlertWebtoon();
  const { trigger: deleteAlert } = useDeleteAlertWebtoon();
  ...
  const renderAlertButton = useCallback(
    (item: Webtoon) => (
      <button onClick={() => handleAlertToggle(item)} aria-label='알람 설정 토글'>
        <PiBellFill className={alertMap.has(item.id) ? "text-yellow-400" : "text-black/30"} />
      </button>
    ),
    [alertMap, handleAlertToggle]
  );
