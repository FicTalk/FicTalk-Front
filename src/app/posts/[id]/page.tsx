...
const EditBtn = ({ id, post }: { id: string; post: { username: string } }) => {
	...
	return data.name === post.username ? (
		<Link href={`/posts/${id}/edit`} className="transition h-fit bg-white text-black/80 hover:text-emerald-500 rounded p-1 text-sm">
			<Edit />
		</Link>
	) : null;
};
...
const DeleteBtn = ({ post }: { post: { username: string } }) => {
	...
	return data.name === post.username ? (
		<button onClick={onClick} disabled={isMutating} className="h-fit text-black/80 p-1 hover:text-red-500 rounded text-sm">
			<Trash2 />
		</button>
	) : null;
};
		...
		<Container>
			<div className="border-b">
				...
				<div className="flex mt-5">
					...
					<div className="self-end">{post.isLiked === null ? null : post.isLiked ? <PostUnLikeBtn /> : <PostLikeBtn />}</div>
				</div>
			</div>
